...
async function handleContact({ request, env }) {
  ...
  const mailPayload = {
    personalizations: [ { to: [{ email: 'info@marareih.org' }] } ],
    from: { email: 'noreply@marareih.org', name: 'Mara Language Preservation' },
    reply_to: { email, name },
  ...
  try {
    ...
    try { mailResult = await mailRes.json(); } catch (e) { mailResult = null; }

    if (!mailRes.ok) {
      console.error('MailChannels failed:', mailRes.status, mailResult);
      let mailText = null;
      try { mailText = await mailRes.text(); } catch (e) { mailText = null; }
      return new Response(
        JSON.stringify({ success: false, error: 'Email provider error', provider: { status: mailRes.status, statusText: mailRes.statusText, json: mailResult, textSnippet: mailText ? mailText.slice(0,120) : null } }),
        { status: 502, headers: { 'Content-Type': 'application/json' } }
      );
    }
