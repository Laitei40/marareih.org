...
<title>Calendar Admin – marareih.org</title>
<style>${getCSS()}</style>
</head>
<body>
...
<div id="dashboard">
  <header class="topbar">
    <div class="topbar-left">
      ...
      <h1 class="topbar-title">Calendar Admin</h1>
    </div>
  ...
  <div class="main-layout">
    ...
    <main id="content" class="content">
      <div id="welcome-panel" class="panel">
        <h2>Welcome</h2>
        <p>Select a calendar from the sidebar or create a new one to get started.</p>
      </div>
...
/* SIDEBAR */
.sidebar{width:var(--sidebar-w);min-width:var(--sidebar-w);background:var(--surface);border-right:1px solid var(--border);overflow-y:auto;transition:margin .25s}
...
.calendar-list li{padding:10px 16px;cursor:pointer;border-left:3px solid transparent;transition:all .15s;display:flex;align-items:center;justify-content:space-between}
.calendar-list li:hover{background:var(--bg)}
.calendar-list li.active{background:#e8f0fe;border-left-color:var(--primary);font-weight:500}
.calendar-list li .cal-name{flex:1;overflow:hidden;text-overflow:ellipsis;white-space:nowrap}
.calendar-list li .cal-count{font-size:12px;color:var(--text-muted);background:var(--bg);padding:2px 8px;border-radius:10px}
...
(function() {
  ...
  function bindEvents() {
    ...
    $$('.modal-overlay').forEach(ov => {
      ov.addEventListener('click', () => {
        ov.closest('.modal').style.display = 'none';
      });
    });
  ...
  function renderCalendarList() {
    ...
    list.innerHTML = calendars.map(c => 
      '<li data-id="' + c.id + '" class="' + (c.id === currentCalId ? 'active' : '') + '">' +
        '<span class="cal-name">' + esc(c.name) + '</span>' +
      '</li>'
    ).join('');
    list.querySelectorAll('li[data-id]').forEach(li => {
      li.addEventListener('click', () => selectCalendar(parseInt(li.dataset.id, 10)));
    });
  }
  ...
  async function selectCalendar(id) {
    ...
    const icsUrl = API + '/api/calendar/' + cal.slug + '.ics';
  ...
  async function loadEvents(calId) {
    try {
      const data = await api('/api/calendars/' + calId + '/events');
  ...
  function renderEvents() {
    ...
    list.innerHTML = currentEvents.map(ev => {
      const start = ev.all_day ? formatDate(ev.start_datetime) : formatDateTime(ev.start_datetime);
      const end = ev.all_day ? formatDate(ev.end_datetime) : formatDateTime(ev.end_datetime);
    ...
    list.querySelectorAll('.edit-event-btn').forEach(btn => {
      btn.addEventListener('click', () => openEditEventModal(parseInt(btn.dataset.id, 10)));
    });
    list.querySelectorAll('.delete-event-btn').forEach(btn => {
      btn.addEventListener('click', () => confirmDeleteEvent(parseInt(btn.dataset.id, 10)));
    });
  }
  ...
  function openEditCalendarModal() {
    const cal = calendars.find(c => c.id === currentCalId);
    if (!cal) return;
  ...
  async function handleCalendarSave(e) {
    e.preventDefault();
    const id = $('#cal-edit-id').value;
    const body = {
      name: $('#cal-name').value.trim(),
      slug: $('#cal-slug').value.trim(),
    ...
    try {
      if (id) {
        ...
        const data = await api('/api/calendars', { method: 'POST', body: JSON.stringify(body) });
  ...
  function confirmDeleteCalendar() {
    const cal = calendars.find(c => c.id === currentCalId);
    if (!cal) return;
    showConfirm('Delete calendar "' + cal.name + '" and all its events?', async () => {
      try {
  ...
  function openEditEventModal(id) {
    ...
    // Recurrence
    if (ev.recurrence) {
      const preset = $('#evt-recurrence-preset');
      const match = Array.from(preset.options).find(o => o.value === ev.recurrence);
      if (match) {
        preset.value = ev.recurrence;
        $('#evt-recurrence').style.display = 'none';
      } else {
        preset.value = 'custom';
        $('#evt-recurrence').style.display = '';
        $('#evt-recurrence').value = ev.recurrence;
      }
    } else {
  ...
  async function handleEventSave(e) {
    e.preventDefault();
    const id = $('#evt-edit-id').value;

    let recurrence = '';
    const presetVal = $('#evt-recurrence-preset').value;
    if (presetVal === 'custom') {
      recurrence = $('#evt-recurrence').value.trim();
    } else {
      recurrence = presetVal;
    }
  ...
  function confirmDeleteEvent(id) {
    const ev = currentEvents.find(e => e.id === id);
    if (!ev) return;
    showConfirm('Delete event "' + ev.title + '"?', async () => {
      try {
  ...
  function toggleAllDay() {
    const allDay = $('#evt-allday').checked;
    if (allDay) {
      $('#evt-start').type = 'date';
      $('#evt-end').type = 'date';
    } else {
      $('#evt-start').type = 'datetime-local';
      $('#evt-end').type = 'datetime-local';
    }
  }

  function toggleCustomRrule() {
    const val = $('#evt-recurrence-preset').value;
    if (val === 'custom') {
      $('#evt-recurrence').style.display = '';
      $('#evt-recurrence').focus();
    } else {
      $('#evt-recurrence').style.display = 'none';
      $('#evt-recurrence').value = val;
    }
  }
  ...
  function copyIcsLink() {
    const url = $('#cal-ics-link').href;
    navigator.clipboard.writeText(url).then(() => toast('ICS link copied!', 'info'));
  }
